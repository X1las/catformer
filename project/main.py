...
class Game:
    ...
    def collisions_rayIntersect(self):
        ...
        corners = [
            Vec(-self.player.width/2, -self.player.height),
            Vec(self.player.width/2, -self.player.height),
            Vec(-self.player.width/2, 0),
            Vec(self.player.width/2, 0)]

        hit = False

        for corner in corners:
            intersect = self.player.rayIntersect(corner , self.rayIntersecters)
            if intersect:
                if intersect[1].length() - self.player.pos.length() < tempLen:
