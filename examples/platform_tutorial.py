...
FramePerSec = pygame.time.Clock()   # creates a Clock object

displaysurface = pygame.display.set_mode((width, height))
pygame.display.set_caption("Game")

# Player class
class Player(pygame.sprite.Sprite):             # Player is a subclass of Sprite
    def __init__(self):
        super().__init__()                      # Sprite is the superclass
        self.surf = pygame.Surface((30, 30))    # creates surface of player?
    ...
    def move(self):
        ...
        # "screen warping" (walking off the screen)
        if self.pos.x > width:
            self.pos.x = 0
        if self.pos.x < 0:
            self.pos.x = width
...
all_sprites = pygame.sprite.Group()     # group of all the sprites
all_sprites.add(PT1)
all_sprites.add(P1)
